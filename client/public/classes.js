...
class Objeto {
    constructor (tamanho, posicao, cor) {        
        ...
        if (this.posicaoInicial.x > cH/2) this.posicaoInicial.x -= this.largura;
...
class Bola extends Objeto {
    constructor (tamanho, posicao, cor, aceleracao = ACELERACAO_INICIAL) {
        ...
        this.posse = -1;
        if (Math.random() < 0.5) this.posicao.x = cW - this.posicao.x - this.largura;
...
class Raquete extends Objeto {
    ...
    static mover() {
        this.all.map(raquete => {
            ...
            if (raquete.movimento.e)
                if(raquete.posicao.x > raquete.limites.e) raquete.posicao.x -= raquete.velocidade.x
                else raquete.posicao.x = raquete.limites.e;
            if (raquete.movimento.d)
                if (raquete.posicao.x < raquete.limites.d) raquete.posicao.x += raquete.velocidade.x
                else raquete.posicao.x = raquete.limites.d;
            if (raquete.movimento.c)
                if(raquete.posicao.y > 0) raquete.posicao.y -= raquete.velocidade.y
                else raquete.posicao.y = 0;
            if (raquete.movimento.b)
                if(raquete.posicao.y < cH - raquete.altura) raquete.posicao.y += raquete.velocidade.y
                else raquete.posicao.y = cH - raquete.altura;
    ...
    checarColisao(bola, j1, j2, excecao = this.movimento.super) {
        ...
        const BORDA_COLISOR = bola.posicao.x + bola.largura;
        const THIS_FUNDO = this.posicao.y + this.altura;
        const FUNDO_COLISOR = bola.posicao.y + bola.altura;
